...
2. **Plomería del 2º canal** — OAuth ✅ **HECHO Y VERIFICADO** (2026-08-06): canal **"Oddly Loop"** (`@oddlyloophq`, `UC6HjFkzmP0LlXeNtK7yMQgQ`), secrets `YT2_CLIENT_ID/SECRET/REFRESH` guardados; `verify_channel2.yml` confirmó que el token lee Oddly Loop. Falta: namespacing en R2 (`channel/auto2/…`, `niche_map.json`) + config por canal en los scripts de subida.
   - ✅ **Capa legal construida** (no depende del OAuth): `channel/auto2/sources.json` = **lista blanca grande** (16 fuentes: Pexels, Pixabay, Coverr, Mixkit, Videvo, Mazwai, Vidsplay, Dareful, Internet Archive, NASA, NOAA, Library of Congress, Prelinger, Wikimedia, YouTube-CC, Vimeo-CC) por nicho, con licencias permitidas y **prohibidas** (bloquea CC-BY-SA/CC-NC + Twitch/Kick/reuploads). + `pipeline/compliance_check.mjs` = **puerta que BLOQUEA publicar** si un clip no está licenciado o la pieza no es transformadora. Probada.
   - ✅ **Ensamblador construido** (`pipeline/build_compilation.mjs`): baja clips de fuentes legales (Pexels/Pixabay) + une con transiciones + narración IA + música + subtítulos + número de ranking; **emite `compilation_manifest.json`** que valida la puerta antes de publicar. Formato 16:9 o 9:16. Contrato assembler→puerta validado; el render ffmpeg se prueba en CI al conectar el canal.
