# Sistema de crecimiento (Oddly Loop + The Data Lens)

Cómo el sistema decide qué producir y qué escalar. Objetivo: **llegar a monetizar** por la ruta de
Shorts (10M vistas en 90 días) o la estándar (1.000 subs + 4.000 h). El ciclo es
`investigar → idear → priorizar → crear → publicar → medir → analizar → aprender → iterar → escalar`,
y cada paso vive en un workflow. Todo en la nube (GitHub Actions + R2 + Telegram).
...
## Fase 2 — Score universal y matriz de outliers (hecho)
...
## Fase 3 — A/B sistemático y banco de creativos (hecho)
...
## Fase 5 — Cruce de evidencia externa e interna (hecho)
...
Las fases 0 a 5 están en producción. Lo que falta de cada una queda anotado en su propia sección.
...
- **Nunca "esto se hará viral"**: se habla de potencial, probabilidad y evidencia.
