...
Auditoría del flujo completo (workflows, scripts y el Worker) para responder una sola pregunta:
**¿funciona 24/7 sin quedarse trancado, colapsar o quedar en limbo?** Cerrada el 2026-08-05; lo de
abajo es el mapa de fallos que dejó y cómo se cerró cada uno.
...
| Señal | Estado | Nota |
|---|---|---|
| Watchdog | 27/27 éxitos | puntual en cada corrida |
...
Severidad: **bloqueante** (rompe el 24/7) · **degrada** (limbo o pérdida parcial) · **menor**.
...
**Arreglo:** Google Cloud Console → *APIs & Services → OAuth consent screen* → si dice *Testing*, botón **“PUBLISH APP → In production”**. Con eso los tokens dejan de expirar.
...
**Arreglo aplicado:** las miniaturas se resuelven **una vez** dentro de `inventory_cache` (cacheado 10 min), no en cada request. Además `r2Usage` se acotó (cache 6h, tope 12 páginas) y `r2json` es resiliente a blips de R2.

### Degradan o dejan en limbo
...
**Arreglo aplicado:** ventana de rescate ampliada a **6 h**; y los disparos entre workflows (daily→produce, guion→voz, regen-QA→voz) ahora **reintentan** (4 intentos), que era donde la cadena se rompía en silencio con `|| true`.

**4. Sin reintento el mismo día** — *código*
Un solo cron de producción (10:00 UTC); un fallo antes del render perdía el día.
**Arreglo aplicado:** **cron de rescate a las 15:00 UTC**. Las guardas (`render_pending` / producción en curso) evitan pisar: si la mañana dejó algo, el rescate no hace nada.
...
**6. El PAT de GitHub o los crons pueden morir en silencio** — **resuelto**
...
`youtube_upload.mjs` reintenta token + init + PUT hasta 3 veces con backoff en 429/5xx/corte; distingue error permanente (4xx de credenciales) de transitorio. (`invalidPublishAt` ya se manejaba de forma blanda.)
...
3. ~~Check del PAT de GitHub y reintento de subida a YouTube~~ — hecho.
